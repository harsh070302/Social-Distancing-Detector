...
net = cv2.dnn.readNet("yolov3.weights", "config.txt")
...
def isclose(p1, p2):
    ...
    #If p1 and p2 are too close then return 1
    # if the distance is medium then return 2
    # else 0.(dangerously close)
...
while(cap.isOpened()):
 ...
 # adding text to image            
    ...
    output = cv2.VideoWriter('output4.avi',fourcc, 10.0, (img.shape[1], img.shape[0]))
